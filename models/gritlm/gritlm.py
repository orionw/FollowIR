...
class GritLM(torch.nn.Module):
    ...
    def encode_queries(self, queries: Union[List[str], str], **kwargs) -> np.ndarray:
        ...
        instructions_list_by_queries = [instructions[q].strip() for q in queries]
        del kwargs["instructions"]
        formatted_instructions_list = [gritlm_instruction_format(i) for i in instructions_list_by_queries]
        return self.encode(queries, instructions=formatted_instructions_list, embed_instruction=True, **kwargs)
    ...
    def encode_corpus(self, corpus: Union[List[str], str, List[Dict[str, str]]], **kwargs) -> np.ndarray:
        ...
        instructions = [gritlm_instruction_format("") for _ in corpus]
        return self.encode(corpus, instructions=instructions, **kwargs)
    ...
    def encode(
        ...
        **kwargs,
        ...
        for start_index in tqdm(range(0, len(sentences), batch_size), desc="Batches", disable=len(sentences)<256):
            sentences_cur = sentences[start_index:start_index + batch_size]
            instructions_cur = instructions[start_index:start_index + batch_size]
            sentences_batch = [
                ((i + " " + s).strip() + self.embed_eos).strip() for s, i in zip(sentences_cur, instructions_cur)
            ]
            if FIRST_PRINT:
                print(sentences_batch[0])   
                FIRST_PRINT = False
            # strip them
            sentences_batch = [s.strip() for s in sentences_batch]
