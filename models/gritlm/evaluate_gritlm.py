...
if __name__ == '__main__':
    ...
    if args.instruction_set != "e5" or args.no_instruction:
    ...
    output_folder = args.output_dir if args.output_dir else f"results/{model_name}"
    ...
    for task_name in task_names:
        ...
        #     print(f"{model_name} instruction for {task_name}: ", instruction)
            # if isinstance(instruction, dict):
            #     model.encode_queries = partial(model.encode_queries)
            #     model.encode_corpus = partial(model.encode_corpus)
            # else:
