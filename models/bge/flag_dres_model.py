...
class FlagDRESModel(DRESModel):
    ...
    def encode_queries(self, queries: List[str], **kwargs) -> np.ndarray:
        ...
        instruction_list = [self.query_instruction_for_retrieval if i.strip() == "" else i for i in instruction_list]
        input_texts = ['{}: {}'.format(i, q).strip() for i, q in zip(instruction_list, queries)]
        print(input_texts[0])
        # else:
        #     input_texts = queries
    ...
    def encode(self, sentences: List[str], **kwargs) -> np.ndarray:
        ...
        all_embeddings = []
        for start_index in tqdm(range(0, len(sentences), self.batch_size), desc="Batches", disable=len(sentences)<256, leave=False):
